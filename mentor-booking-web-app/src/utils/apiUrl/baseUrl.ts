...
export const GET_MENTORS = (page: number, size: number) =>
  `/mentors?page=${page}&size=${size}`;
export const UPDATE_MENTOR = "/mentors/profile";
export const GET_MENTOR_DEGREES = (
  mentorId: string,
  page: number,
  size: number
) => `/mentors/${mentorId}/degrees?page=${page}&size=${size}`;

export const REQUEST = "/requests";

export const GET_BUSY_TIMES =
  "/calendar-events/busy-event?MentorId={mentorId}&Day={day}";
...
export const MAJOR_API_URL = (
  id: string | undefined,
  query: PageRequestModel | undefined
) => {
  //get pagination
  if (query) {
